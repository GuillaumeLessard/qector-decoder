...
- **Email**: [admin@qector.store](mailto:[admin@qector.store](mailto:admin@qector.store))
...
| **GPU acceleration not detected** | The base PyPI wheel includes CPU and standard GPU fallbacks. For specialized enterprise GPU deployments, contact `[admin@qector.store](mailto:admin@qector.store)` for custom build options. |
