"""Extended PyMatching parity (section 3): ``memory_z`` basis.

Mirror of the ``memory_x`` parity suite for the complementary logical basis.
QECTOR's ``pymatching_compat.Matching`` (collapsed graphlike DEM) must match the
reference ``pymatching.Matching`` on IDENTICAL seeded Stim shots: overlapping
...
def _assert_parity(d, shots, seed, basis="z", noise=0.005):
...
def test_pymatching_parity_memory_z(d):
    """QECTOR LER overlaps PyMatching Wilson CI on identical memory_z shots."""
    _assert_parity(d, shots=3000, seed=20260624, basis="z", noise=0.005)
