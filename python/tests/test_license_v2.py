...
def test_v2_roundtrip_carries_tier(signing_key):
    tok = lic.create_license_token_v2(
        "cs_live_1", "Buyer@Example.COM", "solo_perpetual", private_key=signing_key
    )
...
def test_v2_expiry_is_enforced(signing_key):
    expired = lic.create_license_token_v2(
        "cs_live_2", "a@b.com", "evaluation",
        expires_at=time.time() - 60, private_key=signing_key,
    )
    ...
    live = lic.create_license_token_v2(
        "cs_live_3", "a@b.com", "evaluation",
        expires_at=time.time() + 3600, private_key=signing_key,
    )
...
def test_v2_tampered_claims_rejected(signing_key):
    ...
    tok = lic.create_license_token_v2(
        "cs_live_4", "a@b.com", "evaluation",
        expires_at=time.time() - 1, private_key=signing_key,
    )
...
def test_v2_fails_closed_on_pre_v2_verifier(signing_key):
    ...
    tok = lic.create_license_token_v2(
        "cs_live_7", "a@b.com", "solo_annual", private_key=signing_key
    )
