...
def _pymatching_row(code, p, shots, seed):
    ...
    try:
        ...
        t0 = time.perf_counter(); pred = np.asarray(pm.decode_batch(det.astype(np.uint8)), np.uint8); dt = time.perf_counter()-t0
        err = int(np.any(pred.reshape(len(det), -1) != obs.reshape(len(det), -1), axis=1).sum())
        lo, hi = wilson_ci(err, shots)
        return {"decoder": "pymatching", "ler": err/shots, "ci95": (round(lo, 6), round(hi, 6)),
                "decodes_per_s": round(shots/dt, 1), "shots": shots, "errors": err}
...
def _threshold_row(code_name, distance, p, shots, seed):
    res = run_threshold_sweep(
        ...
        decoder="blossom", shots=shots, seed=seed,
    )
    ...
    if args.all:
        ...
        if args.json:
            ...
            for r in results_list:
                if "error" in r: print(f"  {r['decoder']:<20} ERROR: {r['error']}"); continue
                lo, hi = r.get("ci95_lo", 0), r.get("ci95_hi", 0)
                print(f"  {r.get('code','?'):<22} {r['decoder']:<18} LER={r['ler']:.5f} ({lo:.5f},{hi:.5f}) {r.get('decodes_per_s',0):>10,.0f}/s")
        return 0
