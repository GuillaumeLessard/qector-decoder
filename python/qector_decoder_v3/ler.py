...
def estimate_ler(
    ...
    while done < shots:
        ...
        if C.shape != E.shape:
            raise RuntimeError(
                f"decoder returned shape {C.shape}, expected {E.shape} — "
                "refusing to compute a LER on mismatched output"
            )
...
def reference_validate(
    code, decoder_spec: str, p: float, shots: int, seed: int = 0,
) -> dict:
    """E3: validates QECTOR against PyMatching on identical syndromes."""
    import pymatching
    import stim
    d = getattr(code, "distance", 5) or 5
    circ = stim.Circuit.generated("surface_code:rotated_memory_x", distance=d, rounds=d,
    ...
    return {"decoder": decoder_spec, "code": code.name, "p": p, "shots": shots,
        "pymatching_ler": pm_err/shots, "qector_ler": q_err/shots,
        "pymatching_ci95": (pm_lo, pm_hi), "qector_ci95": (q_lo, q_hi),
        "ci_overlap": overlap, "verdict": "PASS" if overlap else "FAIL"}


def run_memory_experiment(
    distance: int, rounds_list: list[int], p: float, decoder: str = "qector_blossom",
    max_shots: int = 40000, max_errors: int = 800, workers: int = 4,
) -> dict:
    ...
    for r in rounds_list:
        circ = stim.Circuit.generated("surface_code:rotated_memory_x", distance=distance, rounds=r,
            after_clifford_depolarization=p, before_measure_flip_probability=p,
            after_reset_flip_probability=p)
    ...
    stats = sinter.collect(num_workers=workers, tasks=tasks, decoders=[decoder],
        custom_decoders=custom, max_shots=max_shots, max_errors=max_errors)
    results = [{"rounds": st.json_metadata["rounds"], "shots": st.shots, "errors": st.errors,
        "ler": st.errors/st.shots if st.shots else float("nan"),
        "ci95": wilson_ci(st.errors, st.shots), "seconds": st.seconds} for st in stats]
...
def run_competitive_suite(
    ...
    for d in (3, 5):
        ...
        try:
            ...
            circ = stim.Circuit.generated("surface_code:rotated_memory_x", distance=d, rounds=d,
                ...
                after_reset_flip_probability=p)
            dem = circ.detector_error_model(decompose_errors=True)
            det, obs = circ.compile_detector_sampler(seed=seed + d * 17).sample(
                shots=shots, separate_observables=True)
            pm = pymatching.Matching.from_detector_error_model(dem)
            t0 = time.perf_counter(); pred = np.asarray(pm.decode_batch(det.astype(np.uint8)), np.uint8)
            dt = time.perf_counter() - t0
            err = int(np.any(pred.reshape(len(det), -1) != obs.reshape(len(det), -1), axis=1).sum())
            lo, hi = wilson_ci(err, shots)
            results.append({"decoder": "pymatching", "code": f"rotated_surface_d{d}",
                "physical_error_rate": p, "shots": shots, "errors": err, "ler": err/shots,
                "ci95_lo": lo, "ci95_hi": hi, "decodes_per_s": round(shots/dt, 1)})
    ...
    try:
        ...
        lo, hi = wilson_ci(err, shots)
        results.append({"decoder": "ldpc_bposd_cs0", "code": "bb72_x", "physical_error_rate": p * 6,
            "shots": shots, "errors": err, "ler": err/shots, "ci95_lo": lo, "ci95_hi": hi,
