#!/usr/bin/env python3
"""Generate QECTOR Decoder v3 User Manual (v0.6.4 Edition) as PDF."""
...
OUTPUT = os.path.join(os.path.dirname(__file__), "QECTOR_Decoder_v3_User_Manual_v064.pdf")
...
def part_divider(label, title):
    return [
        Spacer(1, 6*mm),
        Table(
            [["", Paragraph(f"<b>{label}</b>", ParagraphStyle("PartLabel", parent=s_body, fontSize=11, leading=14, textColor=C_PRIMARY, alignment=TA_CENTER, spaceBefore=1, spaceAfter=1)), ""]],
        ...
        Spacer(1, 2*mm),
        Paragraph(f"<i>{title}</i>", ParagraphStyle("PartSub", parent=s_body, fontSize=10, leading=13, textColor=HexColor("#666"), alignment=TA_CENTER)),
        Spacer(1, 4*mm),
    ]
...
story.append(Spacer(1, 4*mm))
story.append(H2("v0.6.4 Edition changes"))
story.append(Paragraph(
    "This second edition updates the manual from v0.5.2 to v0.6.4. New content includes: "
...
for item in toc_items:
    story.append(Paragraph(item if item else "&nbsp;", s_body))
story.append(PageBreak())
...
story.append(Paragraph(
    "Every code listing in this book is written against the real, installed API (v0.6.4) and follows "
...
story.append(Paragraph(
    "What this manual is not. It is not a textbook on quantum error correction. It assumes you are "
    "familiar with the basic vocabulary of stabilizer codes, check matrices, and decoding — or are "
    "learning them alongside this guide. Where the manual sketches an algorithm, it does so to help "
    "you choose the right decoder and tune it, not to reproduce the research literature.", s_body))
story.append(Paragraph(
    "The v0.6.4 edition. This second edition updates the manual from v0.5.2 to v0.6.4, adding "
    "coverage of the DecoderPool multi-process pool, the cached decoder factory (get_decoder / "
    "clear_decoder_cache), decode_mmap for out-of-core array decoding, DecodeResult and "
    "decode_with_diagnostics for structured diagnostics, the Workbench benchmarking controller, "
    "BP-OSD decode_timed with wall-clock deadline and convergence cap, AVX2 SIMD transpose on "
    "the CPU batch decoder (1.1M shots/s), Blossom intra-decode Rayon parallelism, feature gates "
    "for gRPC/MCP and OpenCL, and the batch_decode_par() method. It supersedes the broken v0.6.3 "
    "release, whose wheels were built with stale Rust source.", s_body))
...
# ══════════════════════════════════════════════════════════════════════════
# ── Chapter 1 ────────────────────────────────────────────────────────────
story.extend(part_divider("PART I", "Getting Started"))
...
story.append(Paragraph(
    "QECTOR was designed from the ground up to be a platform you can trust for research, prototype "
    "with in an afternoon, and scale to production-sized Monte Carlo studies. Every design decision "
    "flows from a small set of principles:", s_body))
story.append(B("One data structure. Every decoder is built from a check-to-qubits adjacency list and a qubit count. There is no second data model to learn, no separate graph format for matching decoders. The same list you use for UnionFindDecoder also works for BlossomDecoder, SparseBlossomDecoder, StreamingDecoder, and every batch decoder. This uniformity reduces bugs and makes it trivial to swap decoders in a benchmark."))
story.append(B("Uniform API. If you can drive one decoder, you can drive them all: construct with (check_to_qubits, n_qubits), then call decode() for a single syndrome or batch_decode() for a 2-D array. Specialized features (weights, streaming, GPU health) are additive — the core pattern never changes."))
story.append(B("Honest by default. QECTOR decoders do not silently return wrong results. Syndrome faithfulness (the property that H c = s) is verified internally where possible; GPU paths expose is_degraded, total_failures, and gpu_recoveries counters so you know when the device is struggling. PyMatching-compatible decoders are tested for bit-identical output on reference workloads."))
story.append(B("Interoperable. First-class bridges to Stim circuits, detector error models, PyMatching, Sinter, and Qiskit mean you can drop QECTOR into an existing toolchain without rewriting your data pipeline."))
story.append(B("Measurable. The built-in Workbench and BenchmarkSuite produce reproducible, artifact-backed numbers. Every benchmark captures the environment snapshot (CPU, RAM, GPU, OS, library versions, git commit) so a number in a report can always be traced back to the exact build that produced it."))
story.append(B("Performance-optimized. The Rust core uses AVX2 SIMD runtime dispatch, intra-decode Rayon parallelism, zero-allocation hot paths, and pre-reserved data structures. The CPU batch decoder reaches 1.1M shots/s on surface d=3 — compete with GPU throughput for small codes."))
...
story.append(note_box("Version covered",
    "This edition documents version 0.6.4, released July 2026. All code listings have been tested "
...
story.append(C("print(__version__)  # '0.6.4' — now authoritative"))
story.append(PageBreak())
...
story.append(Paragraph(
    "Under the hood, the Rust decoder constructs a matching graph from the check-to-qubits list, "
    "with each check becoming a node and each qubit becoming an edge connecting its two adjacent "
    "checks. Fired checks (syndrome bits equal to 1) are graph nodes that need to be paired up. "
    "Union-Find grows clusters around these nodes and merges them; Blossom finds the minimum-weight "
    "perfect matching. The resulting pairing directly translates to which qubits need correction — "
    "edges in the matching correspond to qubits to flip.", s_body))
story.append(Paragraph(
    "All three decoders returned the same correction for this simple syndrome, but they will differ "
    "on more complex inputs. Union-Find is approximate and fast; Blossom is exact and slower; "
    "FastUnionFind sits in between with SIMD acceleration. For a single-bit syndrome on a repetition "
    "code, the answer is unambiguous: flip the qubit adjacent to the fired check.", s_body))
...
# ══════════════════════════════════════════════════════════════════════════
# ── Chapter 4 ────────────────────────────────────────────────────────────
story.extend(part_divider("PART II", "Core Concepts"))
...
story.extend(part_divider("PART III", "The Decoder Catalogue"))
...
story.append(Paragraph(
    ...
    "in validation. Optional per-edge weights let you encode a noise model. As of v0.6.4, "
...
story.append(Paragraph(
    "The key algorithmic difference. Union-Find grows clusters iteratively and merges them when "
    "they touch — it never backtracks, which gives it near-linear O(n \u03b1(n)) runtime but means "
    "it can settle on a heavier matching than the true minimum. Blossom, by contrast, searches "
    "for alternating paths and can re-route matchings through blossom contraction, guaranteeing "
    "the minimum-weight result at the cost of O(n\u00b3) worst-case runtime. SparseBlossom uses "
    "region-growing heuristics to approach the exact result faster. In practice, Union-Find's "
    "approximation costs about 3\u00d7 higher logical error rate on surface codes at threshold — "
    "a traded-off the documentation is upfront about.", s_body))
...
story.append(tip_box("Intra-decode parallelism",
    "BlossomDecoder in v0.6.4 automatically parallelizes k-NN search across available CPU cores "
    "when the number of defects exceeds 40. This provides near-linear speedup on multi-shot "
    "batches with dense syndromes. The threshold is a compile-time constant (INTRA_PARALLEL_THRESHOLD) "
    "and was chosen to amortize Rayon's scheduling overhead against the cost of distance computation."))
story.append(PageBreak())
...
story.append(Paragraph(
    "BP + ordered-statistics decoding for arbitrary qLDPC check matrices. As of v0.6.4, "
...
story.append(C("c   = bp.decode_timed(syndrome, max_latency_ms=10.0)  # v0.6.4"))
story.append(PageBreak())
...
story.append(H2("8.4 DecoderPool (new in v0.6.4)"))
story.append(Paragraph(
...
story.append(Paragraph(
    "How DecoderPool works. When you construct a DecoderPool, it spawns n_workers child processes "
    "using Python's multiprocessing module. Each worker holds its own Rust decoder instance, so "
    "there is no GIL contention and no shared-state synchronization overhead. The pool distributes "
    "batches of syndromes across workers, collects the results, and returns them as a contiguous "
    "array. The per-shot overhead of IPC means the pool is most beneficial for large batches "
    "(> 100K shots) on systems with the 'fork' start method (Linux/macOS).", s_body))
story.append(Paragraph(
    "Windows behaviour. On Windows, multiprocessing uses the 'spawn' start method, which incurs "
    "a 50–500\u00d7 overhead per process creation compared to 'fork'. DecoderPool detects Windows "
    "at runtime and falls back to single-process Rayon-parallel decoding (via batch_decode_par), "
    "which is always faster than multi-process IPC on that platform. For most Windows workloads, "
    "prefer CPUBatchDecoder.batch_decode_par() or FastUnionFindDecoder.batch_decode() directly "
    "instead of using the pool.", s_body))
...
story.append(H2("8.5 Cached decoder factory (new in v0.6.4)"))
story.append(Paragraph(
    "The get_decoder / clear_decoder_cache / get_decoder_pool functions provide LRU-cached "
    "decoder instances. Repeated construction of identical decoders is free after the first call. "
    "The cache key is a tuple of (checks_tuple, n_qubits, decoder_type), so decoders with different "
    "parameters are cached independently. The cache uses an LRU eviction policy with a default "
    "capacity of 128 entries — adequate for multi-distance sweeps and cross-validation without "
    "memory growth.", s_body))
...
story.append(H2("8.6 decode_mmap: out-of-core decoding (new in v0.6.4)"))
story.append(Paragraph(
    "decode_mmap decodes syndromes directly from a memory-mapped NumPy array file without "
    "loading the entire dataset into RAM — ideal for terabyte-scale Monte Carlo sweeps where the "
    "syndrome array exceeds available memory. The function memory-maps the input and output arrays, "
    "processes the syndromes in chunks, and writes corrections directly to the mapped file. "
    "Progress is reported via a tqdm progress bar when the tqdm package is available.", s_body))
...
story.append(Paragraph(
    ...
    "pooled-buffer path with AVX2 runtime dispatch. In v0.6.4, CPUBatchDecoder.batch_decode() "
...
story.append(Paragraph(
    ...
    "is_available(). Note: OpenCLBatchDecoder is gated behind the 'opencl' Cargo "
...
story.extend(part_divider("PART IV", "Integration & the Ecosystem"))
...
story.append(make_table(
    ["Entry point", "Status in 0.6.4", "Purpose"],
    [
...
story.append(Paragraph(
    ...
    "and get_decoder_info. Note: run_mcp_server, run_grpc_server, and start_metrics_server "
    "are gated behind the 'grpc' Cargo feature and may be None if the wheel was built without it.", s_body))

story.append(Paragraph(
    "The MCP server follows the Model Context Protocol specification: it communicates over "
    "standard input/output using JSON-RPC 2.0 messages. The initialization handshake establishes "
    "capabilities, after which a client can call tools/list to enumerate available tools and "
    "tools/call to invoke them. The MCP surface is the most thoroughly tested service endpoint "
    "and is suitable for integration with AI agents and automated benchmarking pipelines.", s_body))

story.append(note_box("REST/gRPC in 0.6.4",
    ...
    "'grpc' feature at build time and are gated behind it: if the wheel was built without the feature, "
    "run_mcp_server(), run_grpc_server(), and start_metrics_server() will be None. Check with "
    "'from qector_decoder_v3 import run_mcp_server; print(run_mcp_server is not None)' before calling."))
...
story.extend(part_divider("PART V", "Workbench & Benchmarking"))
...
story.extend(part_divider("PART VI", "Practical Guides"))
...
story.append(H2("18.3 Latency profiling with BenchmarkSuite"))
story.append(Paragraph(
    "Beyond throughput, you need to understand the latency distribution of your decoder. "
    "The BenchmarkSuite runs a configurable number of warm-up rounds, then measures decode "
    "latency per shot across a large batch. It reports mean, median (p50), p99, and maximum "
    "latency, which together give a complete picture of decoder performance. A low mean with a "
    "high p99 indicates tail latency — a common issue with Blossom on dense syndromes.", s_body))
story.append(C("from qector_decoder_v3 import BenchmarkSuite"))
story.append(C("bs = BenchmarkSuite(checks, n_qubits, n_samples=50000, seed=42)"))
story.append(C("r = bs.run()"))
story.append(C("print('Mean:', r['latency_mean_us'], 'us')"))
story.append(C("print('p50: ', r['latency_p50_us'], 'us')"))
story.append(C("print('p99: ', r['latency_p99_us'], 'us')"))
story.append(C("print('Max: ', r['latency_max_us'], 'us')"))
story.append(C("print('Throughput:', r['throughput'], 'shots/s')"))

story.append(H2("18.4 v0.6.4 performance benchmarks"))
story.append(Paragraph(
    "The following throughput figures were measured on the reference machine with the v0.6.4 wheel:", s_body))
story.append(make_table(
...
story.append(H2("18.5 Reference environment"))
story.append(Paragraph(
...
story.append(Paragraph(
    "This function demonstrates the five essential steps of any LER study: code construction, "
    "error sampling, syndrome computation, decoding, and logical-error counting. The batch_decode "
    "call processes all shots in a single Rust call — never loop over individual shots in Python.", s_body))
story.append(Paragraph(
    "To extend this study to surface codes, replace codes.repetition_code(d) with "
    "codes.rotated_surface_code(d) and adjust the shot count downward (surface codes have more "
    "qubits and checks, so each shot takes longer). For circuit-level noise, replace the "
    "Bernoulli error model with a Stim circuit and DEM as shown in Chapter 12.", s_body))
story.append(Paragraph(
    "Comparing decoders. To compare UnionFindDecoder against BlossomDecoder, simply swap the "
    "decoder constructor. The rest of the pipeline stays the same because every decoder shares "
    "the same API. This uniform interface makes it trivial to benchmark multiple decoders across "
    "the same error samples.", s_body))
...
story.append(Paragraph(
    ...
    "falling back to hard-decision from current beliefs. The maximum number of BP iterations is "
    "also capped at 50 regardless of the deadline, preventing infinite loops on non-convergent codes.", s_body))

story.append(H2("I installed v0.6.3 but it crashes with 'no attribute LERBenchmark'"))
story.append(Paragraph(
    "The v0.6.3 PyPI wheels were built with stale Rust source (the CI secrets had not been updated). "
    "Upgrade to v0.6.4 which has matching Rust and Python layers: "
    "pip install --upgrade qector-decoder-v3", s_body))

story.append(H2("The installed version is still 0.5.x"))
story.append(Paragraph(
    "Clear the pip cache and force reinstall with --no-cache-dir. If you are on Python 3.14, "
    "note that prebuilt wheels are only available for Python 3.9–3.13. Use Python 3.11 or 3.12.", s_body))

story.append(H2("How do I uninstall the old version?"))
story.append(Paragraph(
    "pip uninstall qector-decoder-v3 followed by pip install qector-decoder-v3. Use "
    "py -3.11 -m pip on Windows if you have multiple Python versions.", s_body))

story.append(H2("pip keeps downloading a .tar.gz source distribution instead of a wheel"))
story.append(Paragraph(
    "This happens when there is no wheel for your platform or Python version. "
    "Use --only-binary :all: to force pip to reject source distributions. If the command fails, "
    "it means no compatible wheel exists for your configuration.", s_body))

story.append(H2("Does QECTOR support Python 3.14?"))
story.append(Paragraph(
    "Not yet. The CI pipeline builds wheels for Python 3.9 through 3.13. Python 3.14 support "
    "will be added in a future release once the PyO3/maturin ecosystem adds 3.14 wheels.", s_body))

story.append(H2("Can I build from source on my own machine?"))
story.append(Paragraph(
    "The Rust source is proprietary and not distributed in the repository or the sdist. Prebuilt "
    "wheels are the only supported installation method. To license the source, contact "
    "admin@qector.store.", s_body))

story.append(H2("How do I cite QECTOR in a paper?"))
story.append(Paragraph(
    "Use the CITATION.cff file in the repository root, or the following BibTeX:", s_body))
story.append(C("@software{lessard2026qector,"))
story.append(C("  author  = {Guillaume Lessard},"))
story.append(C("  title   = {{QECTOR Decoder v3}: Rust/Python Quantum Error Correction Decoding Platform},"))
story.append(C("  year    = {2026},"))
story.append(C("  version = {0.6.4},"))
story.append(C("  url     = {https://github.com/GuillaumeLessard/qector-decoder}"))
story.append(C("}"))
story.append(PageBreak())
...
# ══════════════════════════════════════════════════════════════════════════
# ── Appendix A ───────────────────────────────────────────────────────────
story.extend(part_divider("PART A", "Appendices"))
...
story.append(H2("DecoderPool API (new in v0.6.4)"))
story.append(make_table(
    ["Member", "Meaning"],
    [
...
story.append(H2("Cached factory API (new in v0.6.4)"))
story.append(make_table(
    ["Function", "Meaning"],
    [
...
story.append(Paragraph("Covers version 0.6.4. Questions and commercial licensing: admin@qector.store", s_footer))
...
def add_page_number(canvas, doc):
    ...
    text = f"QECTOR Decoder v3 — User Manual — v0.6.4 — Page {page_num}"
